...
public class Map extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (MainActivity.citylatitudeText == 1000) {
			...
			minLat = gps_lat - 0.05;
			maxLat = gps_lat + 0.05;
			minLng = gps_long - 0.05;
			maxLng = gps_long + 0.05;
		} else {
			HOLDING_LOCATION = CITY_LOCATION;
			minLat = MainActivity.citylatitudeText - 0.05;
			maxLat = MainActivity.citylatitudeText + 0.05;
			minLng = MainActivity.citylongitudeText - 0.05;
			maxLng = MainActivity.citylongitudeText + 0.05;
		}
		...
		for (int i = 0; i < NewSearch.howMany; i++) {
			if(SingleItem.distance()==0){
				...
				map.addMarker(new MarkerOptions().position(RANDOM_LOCATION).title(
						categoryList.get(i) + " This item is for sale in your area").icon(BitmapDescriptorFactory
			...
			RANDOM_LOCATION = new LatLng(randomLat, randomLng);
			map.addMarker(new MarkerOptions().position(RANDOM_LOCATION).title(
					categoryList.get(i) + " Distance: " + SingleItem.distance() + " miles from your current location").icon(BitmapDescriptorFactory
					        .defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
			}
		}
		...
		CameraUpdate update = CameraUpdateFactory.newLatLngZoom(HOLDING_LOCATION, 14);
